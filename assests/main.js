...
if (document.getElementById('loginForm')) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        .then(res => {
            // If backend doesn't implement /login this will often be 404 or non-JSON
            if (!res.ok) throw new Error('No server login');
            return res.json();
        })
        .then(data => {
        ...
        .catch(err => {
            // Fallback: if backend missing, allow local dev credential (admin / 2003)
            if (username === 'admin' && password === '2003') {
                localStorage.setItem('isLoggedIn', 'true');
                msg.style.color = 'green';
                msg.textContent = 'Login successful (local)! Redirecting...';
                setTimeout(() => { window.location.href = 'index.html'; }, 600);
            } else {
                msg.style.color = 'red';
                msg.textContent = 'Server Error or Invalid Credentials!';
            }
        });
